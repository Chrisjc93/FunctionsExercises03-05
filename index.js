...
{
 reversedString = reversedString = strToReverse.split('').reverse().join('');
} else if (typeof(strToReverse) === 'number')
{
   reversedString = Number(String(strToReverse).split('').reverse().join(''));
} else 
{
  reversedString =`Incorrect input`;
}

 return reversedString;
}

console.log(reverseCharacters(1013));
...
function myArrayFunction ( inputArray )
{
  let initalArray = [];

  for (let i = 0; i < inputArray.length; i++)
  {
    initalArray.push(reverseCharacters(inputArray[i]));
    
  }
  initalArray = initalArray.reverse();
  return initalArray;
}

console.log(myArrayFunction(['apple', 'potato', 'Capitalized Words']))
console.log(myArrayFunction(['123, 8897, 42, 1138, 8675309']))
console.log(myArrayFunction(['hello', 'world', 123, 'orange']))
